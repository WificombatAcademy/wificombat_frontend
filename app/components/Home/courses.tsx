            ...
            {coursesData.map((course, index) => (
                <SwiperSlide key={course.id} className='pt-5 md:pt-12 pb-8 relative flex items-center justify-center overflow-visible'>
                    <div className={`inset-0 overflow-visible ${index === activeSlide ? 'absolute' : 'hidden'}`}>
                        <div className='flex-shrink-0 absolute w-full h-[500px] bottom-0 bg-purple-500 left-5 rounded-lg max-md:hidden'></div>
                    </div>
                    <div className='z-[2] relative h-[500px] bg-white p-6 text-2xl shadow-lg rounded-lg'>
                        <div className='w-full h-[240px] bg-black-300 rounded-lg'></div>
                        ...
                        <div className='mt-5'>
                            <h4 className='text-black-500 font-semibold'>{course.title}</h4>
                        </div>

                        <div className='mt-5 md:mt-7'>
                            <Link href={course.link}
                             className={`py-4 px-8 rounded-lg text-base 
                             ${index === activeSlide ? "bg-black-500 text-white" : "text-black border border-black-500 " }`}>
                                View Course
                            </Link>
                        </div>
                    </div>
