...
const Page = () => {
  ...
  const [courses, setCourses] = useState<any[]>([]); // Replace `any` with your actual course type
    ...
    const fetchCourses = async () => {
      ...
      try {
        const coursePromises = dashboardData.course_ids.map((id: string) =>
          axios.get(`${API_VERSION_ONE}/course/${id}/modules/`) // Fetching course modules
        );
        const courseResults = await Promise.all(coursePromises);
        const fetchedCourses = courseResults.map((result) => result.data); // Extract the data
                  ...
                  <div key={ind} className="flex flex-col gap-8">
                    ...
                    <div className="flex items-center justify-between gap-6">
                      ...
                      <div>
                        <Link href={`/dashboard/report-card/course_${course.id}`}>
                          <button className="bg-purple-500 text-white py-3 2xl:py-4 px-3 md:px-4 lg:px-6 
                    ...
                    {/* <div className="flex items-center gap-5 overflow-x-scroll">
                      {course.modules.map((module: any, index: number) => (
                        <div key={index} className="w-[60%] md:w-[50%] lg:w-[25%] flex-shrink-0 mb-5">
                          <Link href={module.unlocked ? `/dashboard/career-path/coding-pathway/module_${index+1}` : ""}>
                            <button disabled={!module.unlocked} className="relative w-full h-[150px] md:h-[190px] lg:h-[215px] disabled:cursor-not-allowed">
                              {!module.unlocked && (
                                <div className="absolute inset-0 bg-[#B1B1B4]/30 rounded-2xl"></div>
                              )}
                              <Image
                          ...
                          <div className="px-1">
                            ...
                            <div className="mt-2 flex flex-wrap gap-3 md:gap-5 text-black-600">
                              ...
                              <div>{module.assignment} {module.assignment > 1 ? "Assignments" : "Assignment"}</div>
                            </div>
                          </div>
                        </div>
                      ))}
